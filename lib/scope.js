...
Scope.prototype.add = function(name, kind, node, referableFromPos, freeFromPosition, originalDeclarator) {
    assert(is.someof(kind, ["fun", "param", "var", "caught", "const", "let"]), kind + " is a wrong kind");
    ...
    if (scope.decls.has(name) && (Scope.options.disallowDuplicated || isConstLet(scope.decls.get(name).kind) || isConstLet(kind))) {
        return error(node.loc.start.line, "{0} is already declared", name);
    }
