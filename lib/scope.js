...
Scope.prototype.add = function(name, kind, node, referableFromPos, freeFromPosition, originalDeclarator) {
    ...
    if ( scope.decls.has(name)
		&& (kind !== 'get' && kind !== 'set')//TODO:: do something to allow getter and setter declaration both be in scope.decls
		&& (Scope.options.disallowDuplicated || isConstLet(scope.decls.get(name).kind) || isConstLet(kind))
	) {
        return error(node.loc.start.line, "{0} is already declared", name);
    }
